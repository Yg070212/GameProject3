...
#pragma region 복사 생성자
	...
	// 기본 복사 생성자가 호출되기 때문에 얕은 복사로 호출됩니다.

#pragma endregion

#pragma region 얕은 복사
	// 객체를 복사 할 때 주소 값을 복사하여 같은
	// 메모리 공간을 가리키게 하는 것 입니다.

	// Unit firebet1(50);

	// Unit firebet2(firebet1);

	// 얕은 복사의 경우 같은 객체가 서로 같은
	// 공간을 참조하고 있기 때문에 하나의 객체 값을
	// 변경하게 되면 서로 참조된 객체도 함께 영향을 받습니다.
#pragma endregion

#pragma region 깊은 복사
	// 객체를 복사할 때, 참조 값이 아닌 객체 자체를 새로
	// 복사하여 서로 다른 메모리를 생성하는 복사 입니다.

	Item posion1('B');

	Item poston2(posion1);
